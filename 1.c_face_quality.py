...
img = cv2.resize(cv2.imread("output/aligned_face.jpg", cv2.IMREAD_COLOR), (224, 224))
img = np.array(img, copy=False, dtype=np.float32)
img = np.expand_dims(img, axis=0)
print(img.shape)
# img = cv2.imread("aligned_face.jpg", cv2.IMREAD_COLOR)
# test_data = np.array(test_data, copy=False, dtype=np.float32)
#Extract quality scores for the samples
score = model.predict(img, batch_size=1, verbose=1)
print(score)